...
;-------------
	...
	CHECK_B
		GETC					;gets input to check
		OUT
		
		ADD R0, R0, R6
		BRz INPUT_WHILE_LOOP	; if 0, ok s continue 
		
		LEA R0, ERROR_1			;prints error message
		PUTS
		ADD R0, R0, #0
		BRnp CHECK_B
	...
	INPUT_WHILE_LOOP		;gets user input and stores into array
		...
		ADD R6, R0, R6		;checks for space which is okay
		BRz INPUT_WHILE_LOOP				
		
		LD R6, DEC_n48		;resets r6 for 0
		ADD R6, R0, R6
		BRz IS_VALID
		LD R6, DEC_n49		;resets to check for 1
		ADD R6, R0, R6
		BRz IS_VALID
		LD R6, DEC_n32		;resets to check for space
		ADD R6, R0, R6
		BRz IS_VALID
		...
		ADD R3, R3, #-1
		BRp INPUT_WHILE_LOOP	;end loop
	
	LD R5, DEC_1
	LD R6, DEC_n48			; set R6 to -48
...
;---------------
	...
	DEC_n32 .FILL #-32
	DEC_n49 .FILL #-49
	ERROR_1  .STRINGZ " Not a 'b', try again\n"
	ERROR_2  .STRINGZ "\nNOT 1, 0, or space\n"
